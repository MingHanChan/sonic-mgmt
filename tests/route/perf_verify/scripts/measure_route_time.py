...
T = (timestamp of the LAST route create) - (timestamp of the FIRST route create)
That window is the headline number: how long orchagent+syncd took to push the
whole batch of routes into the ASIC.
...
CREATE_ACTIONS = {"c", "C"}  # single + bulk create
...
def main():
    ...
    ap.add_argument("--since", default=None,
                    help="only count route creates at/after this timestamp "
        ...
        with open(args.recfile, "r", errors="replace") as f:
            for line in f:
                if "SAI_OBJECT_TYPE_ROUTE_ENTRY" not in line:
                    continue
                fields = line.split("|", 2)
                if len(fields) < 2 or fields[1] not in CREATE_ACTIONS:
                    continue
                m = TS_RE.match(line)
                if not m:
                    continue
    ...
    if count == 0 or first is None:
        if args.quiet:
            ...
            print("No route creates found (check --since / recfile).")
        return
    ...
    if args.quiet:
        ...
        print("routes created : %d" % count)
        print("first create   : %s" % first)
        print("last create    : %s" % last)
        print("programming T   : %.3f ms  (%.3f s)" % (ms, ms / 1000.0))
        if ms > 0:
